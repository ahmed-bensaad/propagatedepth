...
class PixelCL(nn.Module):
    def __init__(
        ...
        self.forward(torch.randn(4, 3, image_size, image_size),torch.randn(4, 1, image_size, image_size))
    ...
    def forward(self, x, depth):
        ...
        coordinates = torch.meshgrid(
            torch.arange(image_h).type_as(x),
            torch.arange(image_w).type_as(x)
        )
       ...
       # print(f'proj_coors_two_expanded {proj_coors_two_expanded.shape}')
        #print(f'proj_coors_one_expanded {proj_coors_one_expanded.shape}')
        
        if self.plusplus:
            depth_one_cutout = cutout_and_resize(depth, cutout_coordinates_one, output_size = proj_image_shape, mode = self.cutout_interpolate_mode) 
            depth_two_cutout = cutout_and_resize(depth, cutout_coordinates_two, output_size = proj_image_shape, mode = self.cutout_interpolate_mode)
            proj_depth_one_cutout, proj_depth_two_cutout = map(lambda t: rearrange(t, 'b c h w ->b (h w) c'), (depth_one_cutout, depth_two_cutout))
            
            proj_depth_one_expanded = proj_depth_one_cutout[:, :, None].expand(-1, num_pixels, num_pixels,-1).reshape(-1, num_pixels * num_pixels,1)
            proj_depth_two_expanded = proj_depth_two_cutout[:, None, :].expand(-1, num_pixels, num_pixels,-1).reshape(-1, num_pixels * num_pixels,1)
        ...
        #print(f'positive_mask_one_two before matmul {positive_mask_one_two.shape}')
        if self.plusplus:
            positive_mask_one_two = torch.matmul((intensity_matrix < self.distance_thres).type_as(distance_matrix), positive_mask_one_two.type_as(distance_matrix) , ).type_as(positive_mask_one_two)
            #print(f'positive_mask_one_two {positive_mask_one_two.shape}')
        positive_mask_two_one = torch.transpose(positive_mask_one_two ,1,2)
        ...
        #print(f'propagated_similarity_two_one {propagated_similarity_two_one.shape}')
        #print(f'positive_mask_one_two {positive_mask_one_two.shape}')
        loss_pixpro_one_two = - propagated_similarity_one_two.masked_select(positive_mask_one_two).mean()
        loss_pixpro_two_one = - propagated_similarity_two_one.masked_select(positive_mask_two_one).mean()
